...
function ViewAnnouncement() {
    ...
    function getDeoName(name) {
        axios.get("http://localhost:5000/divisionalOffice/oneDivisionalOfficeByUsername/jaffoffice")
       .then(res => {
    ...
    const [from_School,setOneSchool] = useState("")
    function getSchoolName(fromSchool){
        axios.get("http://localhost:5000/school/oneSchoolByUsername/"+fromSchool)
        .then(res => {
             setOneSchool(res.data[0].name);
         })
        .catch(error => console.log(error));
        return from_School;
    }
